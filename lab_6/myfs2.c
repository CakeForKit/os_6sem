...
static int cnt_slab = 10;
module_param(cnt_slab, int, 0);
...
static int myvfs_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    sb->s_blocksize_bits = PAGE_SHIFT;  //смещение page_size << page_shift
...
static int __init myvfs_init(void)
{
    ...
    if (NULL == (cache_mem = kmalloc(
        sizeof(struct myvfs_inode *) * MAX_CACHE_SIZE, 
        GFP_KERNEL)))
    {
    ...
    if (NULL == (cache = kmem_cache_create(
        SLAB_NAME, 
