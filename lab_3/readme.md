...
В компах нет единаго модуля, который бы определял загруженной каждго ядра и перераспределял между ними работу. Это слишом большой обьеманализируемой информации, причем выполняемый постоянно - слишком затратно.
