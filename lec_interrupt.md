...
В персональный компьютерах реализована шинная архитектура. Управление внешними устройствми передано контроллерам (адаптерам). Адаптеры как правило располагаются на материнской плате а контролеры входят в состав внешних устройств.
...
В системе существует 1 быстрое прерывание - прерывани еот системного таймера. Он не может быть распараллелен и не выполняться на разных ядрах. Условно выделяется главный процессор который обрабатывает прерывания от системного таймера.
...
В системе есть только 1 быстрое прерывания - прерывание от системного таймера.

Суть: 
Любой обработчик прерывания в системе выполняется на высочайшем уровне приоритера. В результате никакая другая работа в системе выполнятся не может пока не завершится выполнение обработчика прерывания. Это может влиять на отзывчивость системы.
...
struct softirq_action {
	void	(*action)(struct softirq_action *); // функция кот должна выполнятся
};
...
SMP-архитектура - равноправные процессоры. Они все могут || обрабатывать взодящие пакеты
...
NAPI - new api появилось в коде ядра в связи с тем что интенсивность работы с сетью в современных системах возросла. Если каждый входящий пакет будет приходить на сетевую карточку. Сетевая карточка жто девайс, а с девайсами система работает через прерывания, но прерывание это очень затратное действие в системе.

Поступает большое количество пакетов => решение napш - замена прерываний опросом (polling)
...
В NAPI-совместимых драйверах прерывания отключаются когд ана интерфаейс призодит пакет. Обработчик в этом случает только вызывает rx_schedule что гарантирует обработку пакета в дальнейшем.
...
tasklets - не может сериализоваться - один тип tasklet может выполнятся только на 1 процессоре. Но разные tasklets могут выполнятся параллельно.

tasklets не могут использоваться для обработки нагруженных действий, подобных сетевым пакетам.

tasklets являются компромисом между производительностью и простотой использования.
